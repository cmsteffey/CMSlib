...
namespace CMSlib.ConsoleModule
{
    public class ProgressBarModule : BaseModule
    {
        ...
        public long Current
        {
            ...
            set
            {
                lock (statLock) current = Math.Clamp(value, 0, max);
            }
        }
        ...
        public string FillFormat
        {
            get
            {
                lock (fillerLock) return filler;
            }
            set
            {
                lock (fillerLock) filler = value;
            }
        }
        ...
        public void QuickWriteOutput()
        {
            lock (Parent.writeLock)
            {
                ...
                for (int i = 0; i < Height - 2; ++i)
                {
                    ...
                    Parent.Write((FillFormat) + AnsiEscape.SgrBrightBlackBackGround + new string('█', len) +
                                 (len == internalWidth ? "" : fillers[currPos % 2].ToString()) + (AnsiEscape.SgrClear) +
                                 AnsiEscape.SgrBrightBlackBackGround +
        ...
        protected override IEnumerable<string> ToOutputLines()
        {
            ...
            for (int i = 0; i < Height - 2; ++i)
            {
                yield return sb.Clear().Append(AnsiEscape.LineDrawingMode).Append(AnsiEscape.VerticalLine)
                    .Append(AnsiEscape.AsciiMode).Append(FillFormat).Append(AnsiEscape.SgrBrightBlackBackGround)
                    .Append('█', len).Append(len == internalWidth ? "" : fillers[currPos % 2].ToString())
                    .Append(AnsiEscape.SgrClear).Append(AnsiEscape.SgrBrightBlackBackGround)
                    .Append(len < internalWidth ? new string(' ', internalWidth - len - 1) : "")
                    .Append(AnsiEscape.SgrClear).Append(AnsiEscape.LineDrawingMode).Append(AnsiEscape.VerticalLine)
                    .ToString();
            }
