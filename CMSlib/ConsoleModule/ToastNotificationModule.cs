...
namespace CMSlib.ConsoleModule{
    public class ToastNotificationModule : BaseModule
    {
        ...
        public ToastNotificationModule(string title, int x, int y, int width, int height, TimeSpan toastLifetime) : base(title, x,
            y, width, height, Microsoft.Extensions.Logging.LogLevel.None)
        {
            this.toastLifetime = toastLifetime;
            toastShower = new(toastLifetime.TotalMilliseconds);
            toastShower.Elapsed += CheckToasts;
            toastShower.AutoReset = false;
            toastShower.Start();
        }
        ...
        private bool CheckToasts(object sender, System.DateTime signalTime)
        {
            ...
            current = toShow;
            if (toShow is not null) notifications.Remove(current);
            WriteOutput();
        ...
        protected override System.Collections.Generic.IEnumerable<string> ToOutputLines()
        {
            ...
            yield return output.Append(LineDrawingMode).Append(UpperLeftCorner).Append(HorizontalLine, innerWidth).Append(UpperRightCorner).ToString();
            int titleVisLen = validCurrent.Title.VisibleLength();
            yield return output.Clear().Append(VerticalLine).Append(AsciiMode).Append(SgrUnderline).Append(titleVisLen < innerWidth ? validCurrent.Title : validCurrent.Title.Ellipse(innerWidth)).Append(' ', Math.Max(innerWidth - titleVisLen, 0)).Append(SgrNoUnderline).Append(LineDrawingMode).Append(VerticalLine).ToString();
            string[] split = validCurrent.Text.PadToVisibleDivisible(innerWidth).SplitOnNonEscapeLength(innerWidth).ToArray();
            int lineCount = Math.Min(innerHeight - 1, split.Length);
            for(int i = 0; i < lineCount; ++i){
                yield return VerticalLine + AsciiMode + SgrForeGroundColor(246) + split[i] + SgrClear + LineDrawingMode + VerticalLine;
            }
            string emptyLine = VerticalLine + new string(' ', innerWidth) + VerticalLine;
            for(int i = 0; i < (innerHeight - 1) - lineCount; ++i){
                yield return emptyLine;
            }
            yield return output.Clear().Append(LowerLeftCorner).Append(HorizontalLine, innerWidth).Append(LowerRightCorner).Append(AsciiMode).ToString();
        }
