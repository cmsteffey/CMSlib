...
namespace CMSlib.ConsoleModule
{
    public abstract class BaseModule : ILogger
    {
            ...
            Func<TState, Exception, string> formatter)
        {
            ...
            output.Append(
                $"|{eventId.Id.ToString().TableColumn(5, ExtensionMethods.ColumnAdjust.Right)}:{shortName}|{(this is ICategoricalModule cm ? cm.GetCategoryType().FullName : null)}{eventId.Name?.GuaranteeLength(Width - 32)}{AnsiEscape.SgrClear}");
            lock (AddTextLock)
            {
                this.AddText(output
        ...
        {
            if (Parent is null || parentPages is null || !parentPages.Contains(Parent.SelectedPage.Id))
                return;
