...
ModuleManager manager = new(new StdTerminal());
...
pageThree.First().MouseInputReceived += async (sender, eventArgs) =>
{
    if (eventArgs.InputState is ClickInputState state && state.MouseCoordinates != cachedCoords)
    {
        ...
        module?.SetCell(state.MouseCoordinates.X - 1 - module.X, state.MouseCoordinates.Y - 1 - module.Y,
            toggle.Enabled ? AnsiEscape.SgrBrightGreenBackGround + " " : AnsiEscape.SgrBrightRedBackGround + " ");
